...
def run_study(
    datasets: dict[str, tuple[PredictionRecords, list[tuple[str, Any, bool]]]],
    *,
    models: tuple[str, ...] = ("MF", "SAGE"),
    seeds: int = 5,
    epochs: int = 40,
    dim: int = 32,
) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for dataset_name, (records, regimes) in datasets.items():
        for regime, holdout, undirected in regimes:
            row: dict[str, Any] = {"dataset": dataset_name, "regime": regime}
            for model in models:
                aucs, aps = [], []
                for seed in range(seeds):
                    ...
                    aucs.append(out["auc"])
                    aps.append(out["ap"])
                auc_m, auc_s = _agg(aucs)
                ap_m, ap_s = _agg(aps)
                row[f"{model}_auc"] = auc_m
                row[f"{model}_auc_std"] = auc_s
                row[f"{model}_ap"] = ap_m
                row[f"{model}_ap_std"] = ap_s
            row["sage_minus_mf_auc"] = row.get("SAGE_auc", float("nan")) - row.get(
                "MF_auc", float("nan")
            )
            rows.append(row)
            print(
                f"{dataset_name:<10} {regime:<20} "
                f"MF={row['MF_auc']:.3f}  SAGE={row['SAGE_auc']:.3f}  "
                f"delta={row['sage_minus_mf_auc']:+.3f}",
                flush=True,
            )
    return rows


def to_markdown(rows: list[dict[str, Any]], models: tuple[str, ...] = ("MF", "SAGE")) -> str:
    head = "| dataset | regime | " + " | ".join(f"{m} AUC" for m in models) + " | SAGE-MF AUC |"
    sep = "|---|---|" + "---:|" * (len(models) + 1)
    lines = [head, sep]
    for r in rows:
        cells = " | ".join(f"{r[f'{m}_auc']:.3f} ± {r[f'{m}_auc_std']:.3f}" for m in models)
        lines.append(
            f"| {r['dataset']} | {r['regime']} | {cells} | {r['sage_minus_mf_auc']:+.3f} |"
        )
    return "\n".join(lines)
...
def _load_drugcomb(max_rows: int, csv: str | None) -> PredictionRecords:
    path = csv or os.environ.get("HETEROSPLIT_DRUGCOMB_CSV") or str(DATA_DIR / "summary_v_1_5.csv")
    return load_drugcomb_csv(path, max_rows=max_rows, with_label=False)


def _load_movielens() -> PredictionRecords:
    ratings = DATA_DIR / "ml" / "ml-latest-small" / "ratings.csv"
    if not ratings.exists():
        ratings = download_movielens(DATA_DIR / "ml")
    return load_movielens_csv(ratings, with_label=False)
...
def main() -> None:
    ...
    parser.add_argument("--drugcomb-max-rows", type=int, default=120_000)
    parser.add_argument("--drugcomb-csv", default=None)
    parser.add_argument("--out", type=Path, default=None)
    args = parser.parse_args()

    datasets = {
        "DrugComb": (_load_drugcomb(args.drugcomb_max_rows, args.drugcomb_csv), DRUGCOMB_REGIMES),
        "MovieLens": (_load_movielens(), MOVIELENS_REGIMES),
    }
    for name, (records, _) in datasets.items():
        print(f"{name}: {records.n_records} records", flush=True)

    rows = run_study(datasets, seeds=args.seeds, epochs=args.epochs, dim=args.dim)
